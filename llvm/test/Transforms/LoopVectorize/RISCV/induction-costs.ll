...
; CHECK-NEXT:    [[TMP1:%.*]] = call <vscale x 1 x i32> @llvm.stepvector.nxv1i32()
; CHECK-NEXT:    [[TMP2:%.*]] = mul <vscale x 1 x i32> [[TMP1]], splat (i32 2)
; CHECK-NEXT:    [[INDUCTION:%.*]] = add <vscale x 1 x i32> splat (i32 1), [[TMP2]]
; CHECK-NEXT:    br label %[[VECTOR_BODY:.*]]
; CHECK:       [[VECTOR_BODY]]:
; CHECK-NEXT:    [[VEC_IND:%.*]] = phi <vscale x 1 x i32> [ [[INDUCTION]], %[[VECTOR_PH]] ], [ [[VEC_IND_NEXT:%.*]], %[[VECTOR_BODY]] ]
; CHECK-NEXT:    [[VEC_IND1:%.*]] = phi <vscale x 1 x i32> [ [[TMP2]], %[[VECTOR_PH]] ], [ [[VEC_IND_NEXT4:%.*]], %[[VECTOR_BODY]] ]
...
; CHECK-NEXT:    [[TMP5:%.*]] = or <vscale x 1 x i32> [[VEC_IND1]], [[VEC_IND]]
; CHECK-NEXT:    [[TMP6:%.*]] = sext <vscale x 1 x i32> [[TMP5]] to <vscale x 1 x i64>
; CHECK-NEXT:    [[TMP7:%.*]] = getelementptr i8, ptr [[SRC]], <vscale x 1 x i64> [[TMP6]]
; CHECK-NEXT:    call void @llvm.vp.scatter.nxv1p0.nxv1p0(<vscale x 1 x ptr> [[TMP7]], <vscale x 1 x ptr> align 8 [[BROADCAST_SPLAT]], <vscale x 1 x i1> splat (i1 true), i32 [[TMP3]])
; CHECK-NEXT:    [[TMP8:%.*]] = zext i32 [[TMP3]] to i64
; CHECK-NEXT:    [[AVL_NEXT]] = sub nuw i64 [[AVL]], [[TMP8]]
; CHECK-NEXT:    [[VEC_IND_NEXT]] = add <vscale x 1 x i32> [[VEC_IND]], [[BROADCAST_SPLAT3]]
; CHECK-NEXT:    [[VEC_IND_NEXT4]] = add <vscale x 1 x i32> [[VEC_IND1]], [[BROADCAST_SPLAT3]]
