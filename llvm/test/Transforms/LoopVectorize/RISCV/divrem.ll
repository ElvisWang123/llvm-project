...
; CHECK:       loop.header:
; CHECK-NEXT:    [[IV:%.*]] = phi i16 [ -12, [[ENTRY:%.*]] ], [ [[IV_NEXT:%.*]], [[EXIT:%.*]] ]
; CHECK-NEXT:    [[NARROW_IV:%.*]] = phi i8 [ -12, [[ENTRY]] ], [ [[IV_NEXT_TRUNC:%.*]], [[EXIT]] ]
...
; CHECK:       loop.latch:
; CHECK-NEXT:    [[MERGE:%.*]] = phi i32 [ 0, [[LOOP_LATCH]] ], [ [[SD_EXT]], [[THEN]] ]
...
; CHECK-NEXT:    br i1 [[EC]], label [[EXIT1:%.*]], label [[LOOP_LATCH]]
; CHECK:       exit:
; CHECK-NEXT:    ret void
;
