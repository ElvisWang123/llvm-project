...
void VPlanTransforms::optimizeConditionalVPBB(VPlan &Plan) {
  ...
  for (const auto &List : Tries) {
    ...
    // 1. Split current basic block at LR point so that VPConditionalRegionBlock
    // can be added inbetween.
    VPBasicBlock *ParentBB = LR->getParent();
    VPBasicBlock *ContBB = ParentBB->splitAt(LR->getIterator());

    // 2. Create VPBB, VPConditionalRegionBlock and insert it inbetween of
    // ParentBB and ContBB
    ...
    // 3. Copy recipes inot conditional block.
    for (VPRecipeBase *R : Recipes)
      R->moveBefore(*IfBB, IfBB->end());

    // 4. Add the condition in the parent block.
    //    if (any-of(Mask) == 0) {
    //      vector.if.bb
    //        ...
    //    }
    ...
    // Set proper predecessor and successors for new VPBB.
